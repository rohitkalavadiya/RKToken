...
contract('RKToken', function(accounts) {
  ...
  it('initializes the contract with the correct values', function() {
    return RKToken.deployed().then(function(instance) {
      ...
      assert.equal(name, 'RKToken ', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'RKT', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'RKToken v1.0', 'has the correct standard');
    });
  })
